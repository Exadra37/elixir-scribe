...
defmodule ElixirScribe.Template.File.Inject.InjectEExTemplateBeforeModuleEndTest do
  ...
  test "it injects an EEX template before the module end tag", config do
    in_tmp(config.test, fn ->
        ...
        binding =
          ...
          |> ElixirScribe.TemplateBindingAPI.rebuild_binding_template("create", [file_type: :lib_core])

        assert :ok = TemplateFileAPI.inject_eex_template_before_module_end(base_template_paths, "api_function_template.ex", "api_existing_module.ex", binding)
  ...
  test "doesn't inject an EEX template when already exists in the module", config do
    in_tmp(config.test, fn ->
        ...
        assert {:noop, :content_to_inject_already_exists} = TemplateFileAPI.inject_eex_template_before_module_end(base_template_paths, "api_function_template.ex", "api_existing_module.ex", binding)
