defmodule Mix.Tasks.Scribe.Gen.Html do
  ...
  defp generate_new_files(%DomainContract{} = context) do
    ...
    binding =
      TemplateBuilderAPI.build_binding_template(context)
      |> Keyword.merge(inputs: inputs(context.schema))

    TemplateBuilderAPI.copy_from(paths, ".", binding, files)
  ...
  defp build_files_without_action(context) do
    html_template_path = TemplateBuilderAPI.build_path_html_template(context)
  ...
  def inject_routes(%DomainContract{context_app: ctx_app} = context) do
    ...
    TemplateBuilderAPI.inject_content_before_final_end(router_scope, router_file_path)
