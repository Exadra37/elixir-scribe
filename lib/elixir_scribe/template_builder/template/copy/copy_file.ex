defmodule ElixirScribe.TemplateBuilder.Template.Copy.CopyFile do
  @moduledoc false
  ...
  def copy_from(apps, source_dir, binding, mapping) when is_list(mapping) do
    ...
    for {format, file_type, source_file_path, target, action} <- mapping do
      binding = TemplateBuilderAPI.rebuild_binding_template(binding, action, file_type: file_type)
