  ...
  defp build_api_binding(context, binding) do
    Keyword.merge(binding,
      absolute_module_name:
        ElixirScribe.MixGeneratorAPI.build_absolute_module_name(context, file_type: :lib_core),
      aliases:
        ElixirScribe.MixGeneratorAPI.build_absolute_module_action_name_aliases(context, file_type: :lib_core)
    )
  end
  ...
  defp inject_api_functions(context, base_template_paths, binding) do
    ...
    for action <- resource_actions do
      binding =
        ...
        |> MixGeneratorAPI.rebuild_binding_template(action, file_type: :lib_core)
